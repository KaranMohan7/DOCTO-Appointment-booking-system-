...
const adminlogin = async (req, res) => {
  ...
  try {
    ...
    if (
      ...
      const admintoken = jwt.sign({email: email, role: process.env.ADMIN_ROLE}, process.env.JWT_KEY);
      if(!admintoken) return res.json({success: false, message: "Something went wrong"});
      res.cookie("admintoken",admintoken, { httpOnly: true, secure: process.env.NODE_ENV === "production", sameSite: "Strict", expires: new Date(Date.now() + 3 * 24 * 60 * 60 * 1000), })
