...
const Myappointments = () => {
  ...
  const cancelappointment = async (id, userid) => {
    setloading(true);
    try {
      const { data } = await axios.post(`${backendurl}/user/cancelappointment`, { id, userid }, { withCredentials: true });
      if (data.success) {
  ...
  const initpay = (order) => {
    ...
    const options = {
      ...
      handler: async (response) => {
        try {
          const { data } = await axios.post(`${backendurl}/user/verification`, response, { withCredentials: true });
          if (data.success) {
            toast.success(data.message);
            getappointments();
          } else {
  ...
  const paymentrazorpay = async (id) => {
    setloading(true);
    try {
      const { data } = await axios.post(`${backendurl}/user/payment`, { id }, { withCredentials: true });
      if (data.success) {
        initpay(data.order);
      } else {
        toast.error(data.message);
    ...
    <div className="w-full h-screen">
      ...
      {appointments.length > 0 ? (
        appointments.map((item, index) => (
          <div key={index} className="flex flex-col md:flex-row lg:flex-row items-center justify-center w-full px-16 lg:px-20 mb-5 py-2">
            <img className="w-96 md:w-40 lg:w-40 bg-zinc-200 mr-3" src={item.docdata.image} />
            <div className="flex flex-col md:flex-row lg:flex-row justify-between items-center w-full">
              ...
              <div className="flex flex-col justify-center gap-2 mt-3">
                {item.cancelled ? (
                  <span className="text-red-500 font-semibold">Appointment Cancelled</span>
                ) : (
                  <>
                    {item.payment && !item.iscompleted && (
                      <h1 className="text-green-600 font-semibold ml-5">Successfully Paid</h1>
                    )}

                    {!item.payment && !item.cancelled && (
                      <>
                        <button onClick={() => paymentrazorpay(item._id)}
                        ...
                        <button onClick={() => cancelappointment(item._id, item.userid)}
                    ...
                    {item.iscompleted && item.payment && !item.cancelled && (
                      <p className="border-[1px] border-zinc-300 w-40 px-3 py-2 rounded-md text-sm text-center">Completed</p>
                    )}
